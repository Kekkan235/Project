...
class TechnicWindow(Toplevel):
    def __init__(self):
        ...
        self.btn = Button(self, text='Сгенерировать', command=lambda: self.generate_technic())

        self.label.grid(row=0, column=0, padx=5, pady=5)
        self.select.grid(row=0, column=1, padx=5, pady=5)
        self.btn.grid(row=1, column=2, padx=5, pady=5)

    def generate_technic(self):
        val = str(self.select.get())
        order_id = int(val.split(" ")[0])
        print(f'Generating report, {order_id}')
        order = db.get_order(order_id)
        with open(f'technic/technic_{datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")}.txt', 'a') as f:
